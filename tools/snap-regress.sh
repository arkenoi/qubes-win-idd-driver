...
guest_once() { timeout 60 ./tools/qtest run "cd $INC && modeprobe.exe" 2>&1 | tr -d '\r' | grep -E '^\{' | python3 -c "
import json,sys
try:
...
# Poll until the guest matches $1 (or give up): a single sample taken during a
# replug transit reads the PREVIOUS mode and fabricates a snap "regression"
# (2026-08-06 - the agent log showed the correct exact apply).
guest() { # $1 = expected
    local want=$1 g try
    for try in 1 2 3 4 5 6 7 8; do
        g=$(guest_once)
        [ "$g" = "$want" ] && { printf '%s' "$g"; return 0; }
        sleep 3
    done
    printf '%s' "$g"
}
