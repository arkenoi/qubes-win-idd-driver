...
#                            makes this test FAIL, by design. (rcalone | noticeonly | kbonly | rcinfers | trustzero | shapeskip | infoonly | scanall)
set -u
...
target_of() {
    case "$1" in
        rcalone)    printf '%s' "severity 'info' (not actionable)" ;;
...
if [ -n "${WUNA_DEFECT:-}" ]; then
    target_of "$WUNA_DEFECT" >/dev/null || { say "FAIL  unknown WUNA_DEFECT='$WUNA_DEFECT' (rcalone | noticeonly | kbonly | rcinfers | trustzero | shapeskip | infoonly | scanall)"; exit 2; }
...
for knob in rcalone noticeonly kbonly rcinfers trustzero shapeskip infoonly scanall; do
    want=$(target_of "$knob")
