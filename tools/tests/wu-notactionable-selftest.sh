...
#                            makes this test FAIL, by design. (rcalone | noticeonly | kbonly)
set -u
...
if [ -n "${WUNA_DEFECT:-}" ]; then
    target_of "$WUNA_DEFECT" >/dev/null || { say "FAIL  unknown WUNA_DEFECT='$WUNA_DEFECT' (rcalone | noticeonly | kbonly)"; exit 2; }
...
for knob in rcalone noticeonly kbonly; do
    want=$(target_of "$knob")
