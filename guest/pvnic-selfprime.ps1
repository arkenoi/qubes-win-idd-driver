...
# leaves a failing $LASTEXITCODE and, with no explicit exit, PowerShell hands that back as the
# SCRIPT's verdict. A successful priming ("ok":true,"armed":true) therefore reported failure, and
# the installer's check - correctly honouring the exit code since today - refused to arm the
# unplug latch on a guest that had primed perfectly.
# A diagnostic probe must never be able to decide the caller's verdict.
