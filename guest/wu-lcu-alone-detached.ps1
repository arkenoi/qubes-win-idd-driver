...
Write-Output '=== RESULT ==='
$sessionsPending = Test-Path (Join-Path $cbs 'SessionsPending')
$rebootPending   = Test-Path (Join-Path $cbs 'RebootPending')
$pendingXml      = Test-Path 'C:\Windows\WinSxS\pending.xml'
Write-Output ("cbs_sessions_pending = {0}" -f $sessionsPending)
Write-Output ("cbs_reboot_pending   = {0}" -f $rebootPending)
Write-Output ("winsxs_pending_xml   = {0}" -f $pendingXml)
if ($sessionsPending -or $pendingXml) {
    Write-Output 'DIRTY: the kill left a servicing transaction mid-flight. REBUILD the template'
