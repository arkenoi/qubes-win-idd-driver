...
### 2.2 Roster → role mapping (fixed; reuse, never rename, never invent names)
...
| **NET-7** | Standalone immediate attach — **KNOWN GAP, EXPECTED-FAIL**: no latch ⇒ PnP problem 14 (`XENNET Stopped`) until reboot (measured); a second boot is a FAILURE by the owner's rule, so this path does not meet acceptance and is recorded as such — never "passed after reboot". Flips to NET-2 acceptance verbatim when decision D1 lands | any ST2 standalone → recorded | P0-CORE | 15 min |
| **NET-8** | PV throughput benchmark — **owner dependency**: needs a LOCAL endpoint (file served from the netvm or a neighbour qube; fw-net invisible from here). Until it exists no PV throughput number may be quoted (NET-3's external figure disqualified). When available: 3 runs interleaved with a control; expectation O(hundreds of Mbps) | ST4 → ST4 | NET-2, owner endpoint (D2) | 30 min |
...
- **D1** StandaloneVM latch: seed on Standalones or declare AppVM-only network support? NET-7 stays EXPECTED-FAIL until decided.
...
2. **ST0 is never parked** (lifecycle wins over the install matrix's "clone PRISTINE as golden"): a no-qrexec stage cannot be driven or attested, so pristine-start cells (C1/C2/C10/C12) are executed by the answer-stick's firstlogon orchestration inside the R3 run and graded externally (bootwatch/CPU/screenshots) plus post-hoc durable logs.
