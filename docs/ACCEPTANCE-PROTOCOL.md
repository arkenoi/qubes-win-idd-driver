...
| **NET-7** | **Standalone immediate live attach, ZERO reboots.** DECIDED 2026-08-29 (owner: "latch is always on, standaloneVM included") — the templates-only gate is removed and `Install-QwtImproved.ps1` seeds the latch UNCONDITIONALLY for every qube class, so this is no longer a known gap: it carries **NET-2's acceptance criteria verbatim** on a StandaloneVM. Historical: before the fix a Standalone landed in PnP problem 14 (`XENNET Stopped`) until a reboot — that measurement is the seen-to-fail negative control for this cell, and a second boot remains a FAIL, never "passed after reboot". Requires a package built at/after `cace671`. | any ST2 standalone → ST4 | P0-CORE, NET-0, NET-1 | 20 min, 0 GB |
