...
**Payload delivery — the ONLY two sanctioned routes, with mechanics (added 2026-08-30).** This paragraph previously said "push via `qtest push`" and stopped there, leaving the mechanics unstated. That gap got improvised into: a run attached the release ISO as a **cdrom** instead, which is not a route this protocol defines, and burned three attempts on `qvm-device block assign` (which takes effect only at the **next VM start**, so assigning to a running guest succeeds and changes nothing visible). Never attach the release as a cdrom for an install cell. Concretely, as `mgmt/harness/matrix.sh:push_payload` implements it:

1. Build ONE tarball of the release package (`qwt-improved-setup`) and push that single file — not a directory tree, not per-file pushes. `qtest push` is `qvm-copy-to-vm`, so it lands in `C:\Users\user\Documents\QubesIncoming\win-idd-mgmt\` (`INC` in the harness).
2. Extract guest-side into a per-cell directory: `rmdir /s /q C:\<dir>` then `mkdir C:\<dir>` then `tar -xzf %INC%\<tarball> -C C:\<dir>`, and require its `EXTRACT_OK` echo. A stale directory from a previous cell is grading contamination.
3. **Retry the push 3× with 20 s backoff, keeping stderr** (H3.8): the first attempt fails in about one second on a session that has only just answered `echo QREADY` (`rc=46`), and a cell that treats that transient as failure loses a good run.
4. Assert the guest-side `MANIFEST.json` `source.driver_repo_commit` equals the release under test **before** running `install.cmd`. A short or stale push looks exactly like a broken install an hour later.
