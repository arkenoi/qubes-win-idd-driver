...
created with `udisksctl loop-setup -r -f <iso>` (ROOT-FREE — `sudo losetup` is neither available
nor required) that `losetup -l` then shows backed by the intended file and NOT `(deleted)`.
Explicit capability list for every block-binding step in this document:
