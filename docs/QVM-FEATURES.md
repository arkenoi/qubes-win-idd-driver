...
| `service.uac` | Turns UAC on or off in the guest (`EnableLUA`); needs a guest reboot. Absent leaves the guest's own setting alone; `1` forces UAC on, `0` off. **Off is not recommended:** without UAC any code in the qube reaches admin/kernel directly, and that is the surface facing the hypervisor (hypercalls, grant tables, PV drivers). Where the prompt is *drawn* is not configurable — it follows the display mode: seamless shows it as its own dom0 window, fullscreen shows it on Windows' secure desktop as part of the full screen. Read at agent start (`agent/gui-agent/main.c`, QGAUAC) | absent — guest's own setting untouched |
...
| `/qubes-service/gui-fullscreen`, `/qubes-service/enableWinKey`, `/qubes-service/hideGuestTitleBar`, `/qubes-service/gui-agent-debug`, `/qubes-service/uac` | section 1 | section 1 |
