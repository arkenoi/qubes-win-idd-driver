...
| `service.hideGuestTitleBar` | Strips `WS_CAPTION` from guest windows so dom0's decoration is the only header (`main.c:1832-1845`). Read at `perf.c:348` | **on** (set the feature to an empty value to opt out) |
...
| `service.notify-bridge` | The toast **notification bridge** (docs/DESIGN-toast-bridge.md): the agent launches and supervises `notifhost --bridge` in the user session, which forwards toasts to dom0's stock `qubes.Notifications` service — rendered origin-marked and sanitized by dom0 — and suppresses the sending app's Windows banner while (and only while) its dom0 connection is up; a dom0-side dismissal is echoed back so the guest Notification Center stays in sync. **Since 4.3.31 the route is decided PER TOAST by the classifier** (docs/ADR-toasts.md §2): an informational toast is forwarded, a toast carrying real choices stays on the guest-window path with its buttons. `NotifyBridgeAllow` (REG_MULTI_SZ of AUMIDs under the gui-agent config key) is a per-app SHORTCUT — those apps are forwarded without waiting for a verdict; when the value is unset a conservative built-in seed of reliably-informational apps is used — Snipping Tool, Camera, Photos, Security-&-Maintenance, Backup-reminder — verify/extend per guest with `notifhost --dump-aumids`. A toast with no verdict after ~6 s, every toast while the bridge is unhealthy, and every toast on a guest whose wpndatabase schema does not match, keep the ordinary window path — fail-open by construction. Guest-local twin: `NotifyBridge` DWORD. Read at agent start (NOTIFBRIDGE gate) | **on** — allowlisted toasts go to dom0; `service.legacy-toasts` or `service.notify-bridge 0` restores the window path |
...
| `service.notify-errors` | The **secondary error route** (docs/DESIGN-error-notify.md): an ACTION-severity guest fault — de-slice broker missing or down, guest stuck on the sign-in screen in seamless mode, the toast bridge denied notification access, an IDD activation/deactivation that could not reboot — is **also** raised as a dom0 notification through `notifhost --notify-file` (stock `qubes.Notifications`, origin-marked and sanitised by dom0), in addition to the guest log line, never instead of it. Once per distinct error per boot, at most 8 per boot, secret-shaped text refused. **It rides qrexec-agent**: a guest whose qrexec is down gets nothing from it and the log stays the only record. A sibling of `service.notify-bridge`, not the same switch (that one forwards app toasts and suppresses banners; `service.legacy-toasts` does not affect this). Guest-local twin: `NotifyErrors` DWORD. Read at agent start (NOTIFYERR gate); PowerShell scripts read the same pair | **on** — set `service.notify-errors 0` to keep faults in the guest log only |

`service.hideGuestTitleBar` is **on by default since 4.3.21**. It was default-off from 2026-08-17
after caption-stripped windows were seen to vanish from dom0; that failure was reproduced and
root-caused on 2026-09-07 and it was not the restyle. The agent's raise-on-foreground corrective
re-mapped a window on every focus change, dom0 answered by activating it in the guest, the guest's
foreground changed, and the corrective fed itself - ping-ponging between the caption-less windows.
No `MSG_WINDOW_FLAGS` was involved: with debug logging on there was zero window-flags traffic in
either direction while the windows disappeared. The corrective was a slice-era workaround (a
composite-sliced window receives the pixels of whatever covers it); windows with their own
per-window buffer now skip it, composite-fed ones keep it debounced. Re-verified on the original
scenario: 24 focus flips, `QGARAISE sent=0 debounced=0`, no re-maps, all windows still present.
...
| `SeamlessNoScreenGrant` | DWORD, absent → `1` (on) | no dom0 feature — guest-local only |

`service.hideGuestTitleBar` has no registry twin — qubesdb is its only source.

`SeamlessNoScreenGrant` retires the whole-desktop framebuffer grant: dom0 renders the qube
exclusively from per-window grants, and the desktop is never copied into dom0 at all. It is ON by
default and applies to **seamless only** — a guest with `service.gui-fullscreen` on can be asked
for the whole-desktop window, whose image is that very grant, so on such a guest the agent turns
it back off at start and says so (`P2NOGRANT not applied`). Set it to `0` to restore the grant
unconditionally. This is tidiness and correctness, **not** a security or isolation change.
