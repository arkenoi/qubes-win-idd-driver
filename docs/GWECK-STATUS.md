...
| 9 | 35, 38, 44 | `install /idd` → "file not found" | **FIXED**, **NOT SHIPPED** | Three stacked causes: (a) `Start-Process -FilePath 'D:\idd'` asks Windows to launch a file named `idd` — no such file exists on any medium; (b) the `/iddonly` we announced was committed ~3.7 h *after* the 4.3.1 assets were uploaded, so his medium rejects it; (c) the README described pre-4.3.1 semantics. All three now correct in-tree, including the `%HEREQ%` trailing-backslash bug that broke `/iddonly`. He needs a release |
...
| 18 | 54 | Request: a `/noidd` switch | **FIXED**, **NOT SHIPPED** | `/noidd` (plus `/iddoff` recovery and `/iddonly`) implemented and wired end to end; he is still on 4.3.1, which has none of them |
...
| 21 | 58 | "`/noidd` is not yet accepted" | **NOT SHIPPED** (no code defect) | His own JSON says `package_version 4.3.1+agent.c7ccb459aec9`: 4.3.2 was never published, so the switch never reached him. The fix is to cut a release |
