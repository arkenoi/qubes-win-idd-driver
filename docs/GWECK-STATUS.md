...
| 9 | 35, 38, 44 | `install /idd` → "file not found" | **SHIPPED** | Three stacked causes: (a) `Start-Process -FilePath 'D:\idd'` asks Windows to launch a file named `idd` — no such file exists on any medium; (b) the `/iddonly` we announced was committed ~3.7 h *after* the 4.3.1 assets were uploaded, so his medium rejects it; (c) the README described pre-4.3.1 semantics. All three now correct in-tree, including the `%HEREQ%` trailing-backslash bug that broke `/iddonly`. He needs a release |
...
| 18 | 54 | Request: a `/noidd` switch | **SHIPPED + VERIFIED** | `/noidd` (plus `/iddoff` recovery and `/iddonly`) implemented and wired end to end; he is still on 4.3.1, which has none of them |
...
| Issue | Status | Note |
|---|---|---|
| A full-screen **DWM-cloaked** `ApplicationFrameWindow` is mapped to dom0 as an untitled window covering the whole qube | **OPEN — new** | Found while verifying the release. `cloaked=2` means the shell has hidden it; we map it anyway and dom0 paints a blank full-screen rectangle over everything, while apps and qrexec keep working. Strong candidate for #16/#19. Appears on the non-IDD path too. Cannot be fixed with a blanket cloak filter — toasts are cloaked and must be kept (CLAUDE.md 2A-chrome 3c) |
