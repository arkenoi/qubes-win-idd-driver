...
| `keyed-mutex-recovered` / `capture-thread-survives-resize` | 2026-08-31 | **ATTEMPTED AND NOT EARNED** — FI_CAPTURE_EXIT armed on the fault build | green->red->green ran cleanly (unarmed 20 abandonments/46 recreates/0 deaths; armed 0/0/**1 death**; disarmed 9/21/0) but the red does NOT count: a direct grep showed the only line matching the death pattern was the injector's own message, and the pixel half reported 'pixels changed' on all three mode changes. **Both checks stay unproven, and are now known to be vacuous for a silent capture-thread death.** Injector message reworded to stop tripping the check it validates; the mechanism (why 0 abandonments yet changing pixels) is an OPEN question, not a conclusion |
| `borderless-fullscreen-gated` (FI_GATE_MODE2) | 2026-08-31 | **RE-PROVED FROM ONE ARTIFACT** — `FaultGateOff=0x2` on the gate build `f143f0f1` | baseline all four P5 cells green; with the bit set SG2 went **FAIL** while SG4/SG3/SG9 stayed green; with it cleared SG2 went green again. Only a registry value changed between the three runs, so this is strictly stronger than the two-binary SG2 proof. Banners confirm the bit was live (`gateoff=0x2`) |
| `FI_GATE_OFF` (the bypass mechanism) | 2026-08-31 | armed 0x1, 0x2, 0x4 in sequence | each run's startup banner reported the matching `gateoff=` value, and `FiGateOff` announced itself once per bit. The mechanism is validated end to end by MODE2 flipping its cell |
| `or-fullscreen-never-mapped` (SG4) | 2026-08-31 | **NOT EARNED — DEFENCE IN DEPTH** — `FaultGateOff=0x1` | bypassing Mode 1 alone did NOT leak: SG4 went `INVALID-VACUOUS` (the deny line vanished, proving the bypass was in force) but no window mapped, because the override-redirect fullscreen probe is *also* caught by the Mode-2 gate. The property is defended by two independent clauses |
| `start-not-presented` (SG9) | 2026-08-31 | **NOT EARNED — DEFENCE IN DEPTH** — `FaultGateOff=0x4` | direct measurement with the bit live and Start opened: `Start surface not presented` logged **0** times (clause bypassed), `shell surface with no card` logged **5** times, and dom0 showed only the control window. The genuine-open gate rejects the Start surface independently |
| `or-fullscreen-never-mapped` (SG4) | 2026-08-31 | **EARNED, after the check itself was fixed** — `FaultGateOff=0x3` on the gate build `f143f0f1` | armed: the agent logged `msg=MAP,hwnd=0x3C0062,ovr=1` for a 1600x900 override-redirect window -> SG4 **FAIL** (`agent_map=1`); cleared: SG4 green (`agent_map=0`); SG3/SG9 green in both. **The screenshot alone could not see the leak** — dom0 dims showed only the control — so this proof required adding a second witness (no `msg=MAP` for the probe hwnd) to every nomap cell. The same run shows the contrast: SG2's decorated leak was visible to BOTH witnesses, SG4's only to the log |
| **15 rows have NO DEPLOYED CHECK** | 2026-08-31 | swept every unproven check name against `mgmt/`, `tools/`, `guest/` | `agent-idle-cpu`, `appvm-private-reformatted`, `d1-validated-on-a-build`, `d3-settled`, `dialog-window`, `drag-p50-record-only`, `menu-override-redirect`, `metric-stability`, `no-phantom-window`, `no-premature-reboot-dialog`, `owner-window-renders`, `pv-disk-bound`, `scroll-p50-vs-canonical`, `shadow-strips-dropped`, `uac-off-secure-desktop` — nothing emits any of these verdicts. H5 does not apply; they are hand-recorded observations. Left PASS-UNPROVEN on purpose: upgrading them would overstate verification, and 'proving' one would mean writing a new check and then proving THAT, which is a different claim |
| `or-fullscreen-never-mapped` (SG4) — SECOND, ONE-ARTIFACT PROOF | 2026-08-31 | `FaultGateOff=0x3` (MODE1|MODE2) on gate build `fd01cf82` | baseline all four green; armed SG4 **FAIL** with SG2 FAIL as the DECLARED companion (Mode 2 is the second clause defending the same property) and SG3/SG9 green; restored all four green. Only a registry value changed |
| `start-not-presented` (SG9) | 2026-08-31 | **ATTEMPTED, NOT EARNED** — `FaultGateOff=0x14` (START|NOCARD) | bypassing the cardless-surface reject destabilises the WINDOW SET itself: SG4/SG2/SG3 all returned INVALID-INSTRUMENT with `dom0 dims:` EMPTY — even the control window vanished — and SG9 did not return green after clearing. Nothing measured in that state is attributable, so no proof. This route is unusable; the check stays PASS-UNPROVEN |
| `windowed-fullscreen-allowed` (SG3) | 2026-08-31 | **ATTEMPTED, NOT EARNED** — `FI_DROP_CAPTIONED=0x20` | the bit works, but P5's OWN CONTROL is a captioned Notepad, so dropping captioned windows destroys the instrument's control: P5 aborted with *'the control window never became visible … the capture path is blind'*. A bit that disables the harness's control cannot prove anything through that harness — this needs a control that is not captioned |
| `negative-control-normal-window-maps` (SG1) | 2026-08-31 | **EARNED** — `FaultGateOff=0x20` (FI_DROP_CAPTIONED) on gate build `fd01cf82`, through the DEPLOYED harness across real cold boots | armed: `notepad mapped: 0 window(s)` -> **FAIL**; cleared: `notepad mapped: 1 window(s)` -> PASS. Only a registry value differed. This is the first proof of a check asserting a window IS presented - gate-removal cannot falsify those, which is why FI_DROP_* exists |
| `menu-synthesized-onto-owner` (RND-3) | 2026-08-31 | **NOT FALSIFIABLE BY AN ACCEPT-PREDICATE BIT** — `FI_DROP_SHELLSURFACE=0x40` armed | RND-3 stayed PASS (1 SYNTH event, owner pixels changed) while RND-4 went FAIL in the SAME run. Menus are SYNTHESIZED onto their owner's surface, not mapped as windows, so they never reach the ShouldAcceptWindow reject the bit removes. Any proof for this check must target the SYNTHESIS path, not the accept predicate — noting it here so the next attempt does not re-run this bit |
| `toast-reaches-dom0` (RND-4) | 2026-08-31 | **EARNED** — `FaultGateOff=0x40` (FI_DROP_SHELLSURFACE) on gate build `fd01cf82`, through the DEPLOYED cell | armed: 82 guest-side toast samples but dom0 override-redirect stayed 0 and no synth+pixel change -> **FAIL**; cleared -> PASS. RND-3 stayed PASS in the same armed run, so the bit moved exactly the toast path. NOTE: `toasts-survive-filter` could NOT be paired - the armed run never reached that cell, so its absence is not a FAIL verdict and it stays unproven |
| `menu-synthesized-onto-owner` / `owner-window-renders` (RND-3) | 2026-08-31 | **VACUOUS — measured** — `FI_NOSYNTHPAINT=0x80` at the paint chokepoint | banner `gateoff=0x80`, announce 1, **`SYNTHPAINT 0`** (not one synth paint occurred) — and RND-3 still reported *'menu synthesized onto its owner AND the owner's dom0 pixels changed'*. The pixel half cannot tell compositing from any other repaint; the other half counts bookkeeping. **Third vacuous check found by the injector** (after SG4-blind-to-override-redirect and the keyed-mutex death counter). Needs REWRITING, not a proof |
| `mode-followed-1024x768` / `-1600x900` / `-1920x1080` (RND-8) | 2026-08-31 | **EARNED, after the check was repaired** — `FaultGateOff=0x100` (FI_NOSCREENCONFIG) on build `24d75cd1` | armed: all three FAIL (*'the guest adopted <M> but the agent last told dom0 none'*); cleared: all three PASS (*'AGENTSCREEN <M> … they agree'*). **The check was VACUOUS first**: it emitted `guest+agent both report <M>` from inside the pixel branch while `ag` was logged and never compared — with `AGENTSCREEN none` on every mode it still recorded agreement. Fixed, then proven |
| `pixels-change-after-resize-*` (RND-8) | 2026-08-31 | **NOT EARNED** — same armed run | the defect destroyed the capture itself, so all three reported INVALID-INSTRUMENT (*'no capture at <M>'*), not FAIL. INVALID is never folded into a fail-proof. Open limitation: the harness cannot distinguish 'my instrument broke' from 'the product broke my instrument' |
| payload verification (incidental) | 2026-08-31 | **SEEN TO REFUSE, twice, from two different defects** | (a) file deleted but still listed: *`payload verification FAILED: MISSING pvnic-selfprime.ps1`*; (b) `MANIFEST.json` edited: *`MISMATCH MANIFEST.json (got fb598f…, want 7eee93…)`* — the manifest is itself covered by the sums. Both refused BEFORE msiexec; a consistent tree then installed normally. The installer will not install a partial or tampered payload, demonstrated rather than assumed |
| `keyed-mutex-recovered` / `capture-thread-survives-resize` | 2026-08-31 | **EARNED, after the check was repaired** — `FaultCaptureExit=1` on build `24d75cd1` | armed: **`QGAPERF seq 3 -> 3`**, frames not advancing -> FAIL; cleared: `seq 190 -> 237` -> PASS. The old DIED counter was a log grep matching only the injector's own message; frame liveness cannot be satisfied by any log line. **Also settled the open question**: capture WAS dead while the old pixel probe reported 'pixels changed' — that probe never measured live capture |
| `menu-synthesized-onto-owner` (RND-3) | 2026-08-31 | **EARNED, after the check was repaired** — `FaultGateOff=0x80` (FI_NOSYNTHPAINT) on build `24d75cd1` | armed: *'SYNTH but NEVER a SYNTHPAINT — accounted onto its owner and never drawn there'* -> FAIL; cleared: painted rect `1,50 229x196` changed `6f477934 -> 228e6e7d` -> PASS. The old whole-window hash passed with `SYNTHPAINT 0`; evidence is now cropped to the rect the agent itself reports painting |
| `uac-off-secure-desktop` | 2026-08-31 | **PROMOTED then EARNED** — was a hand-recorded observation; now a real check in `promoted-checks.sh`, fail-proofed by setting `PromptOnSecureDesktop=1` | armed: *'elevation prompts go to the SECURE DESKTOP, where seamless mapping is suppressed'* -> FAIL; restored to 0 -> PASS. Reversible registry plant, no build. Guards a silent failure — the qube appears to hang with no prompt anywhere |
| `standalone-pvnic-seeded` + `emulated-unplugged` | 2026-08-31 | **PROMOTED then EARNED** — both were hand-recorded observations; now real checks in `promoted-checks.sh`, fail-proofed on `win10-noprime` (disposable clone, latch tasks + applier + NICS removed, rebooted) | red: `task_main=false task_rearm=false` applier absent -> FAIL, and `Realtek RTL8139C+ … | Up` alongside a disconnected PV NIC -> FAIL. green on `win10-app`: tasks registered, applier `c98dfc40`, only `Xen PV Network Device #0 | Up`. **RETRACTS the earlier 'negative proven unreachable'** — unsetting NICS alone does not stick because the PV stack re-seeds it; removing the TASKS and APPLIER too does |
| `pvnic-latch-readback.ps1` (the probe) | 2026-08-31 | **PROBE DEFECT FOUND AND FIXED** | on the unseeded guest it printed `MARKJSON` and nothing else: `.Hash.ToLower()` on the missing applier threw and killed the whole object, so the check that exists to detect a missing applier could only ever report INVALID-INSTRUMENT for it. Made null-safe. *A probe must be able to describe the defect it was written to find* |
| `shadow-strips-dropped` | 2026-08-31 | **PROMOTED to a real check; GREEN IS VACUOUS** — `FI_GATE_SHELLOVERLAY=0x8` armed, banner `gateoff=0x8` | dom0 still gained exactly 1 window. The agent log gives the reason: chromerepro's strips are rejected by `GetRealWindowRect` as *'inverted DWM bounds (0,0)-(0,0) and no usable GetWindowRect'* — **geometrically degenerate, before any chrome predicate runs**. The cell proves zero-area windows are dropped, NOT that the 2A-chrome filter works. The whole 2A-chrome acceptance rests on this repro. FIX OWED: give the strips real rects |
...
| `w_appvm_shell` stale-private guard | 2026-09-04 | synthetic, rig-free (an acceptance run held the rig): probes stubbed to "console session Active, explorer.exe never" — the stale-private AppVM signature (owner correction: an AppVM's private seeds from the template private AT CREATION only; re-basing the template under a reused AppVM leaves a stale private → autologon but no shell) | `mgmt/harness/failproof-appvm-shell.sh`, five-sided: defect fixture → rc=1 naming *"stale private volume; the AppVM was not re-created after its template was re-based"*; healthy → rc=0; no-session → rc=2 bounded DEADLINE (never misdiagnosed); slow logon → rc=0; the sg6-measured prompt-echo false positive → stripped while a real tasklist line still matches |
| `a0-instrument-floor` (p6-toast-bridge) | 2026-09-05 | **EARNED, LIVE, on a real product defect** — notifhost carrying the AgentGone `CreateToolhelp32Snapshot` liveness probe (diagnostic build `b5bd232`) silently terminated the bridge on ~2/3 of forward→dismiss cycles: the snapshot's variable latency on the sole worker thread pushed a loop iteration past the agent supervisor's 15 s stale-heartbeat deadline, and the supervisor's delete-task-first relaunch killed the still-alive bridge artifact-free (no WER dump, no event, Status=Running). The defect build was RE-RUN repeatedly against the floor — that re-run is the G-2 reintroduction half | floor's bridge-dependent checks red 3×/3 on the crashing build; pre-regression `250b007f` ran the 3-forward reproducer **8/8 clean** on the same guest+agent (the A/B bisect); fix `71d609c` (snapshot-free `OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION)` + 30 s throttle) re-greened it. Registry entry in `protocol/failproofs.json` carries grade **live** |
| `a0-suite-verdicts` / `a0-failopen-proven` / `a0-no-instrument-misses` (p6-toast-bridge) | 2026-09-05 | **DRY-GRADE ONLY** (harvested from `defect-a0-phase-fail` / `defect-a0-failclosed` / `defect-a0-instrument-verdict`, fixtures copied from real observed output grammar). The suite's underlying rows WERE seen red live — P8 `hbseen=1` (the stale pre-boot-heartbeat race, pre-`c8012e2`), P4a false-FAIL (`SENT id=15` hidden by the constant-10 `blog_len`, pre-`f45a65e`), P6a graded an unfired toast (pre-`72f1903`) — but those were **instrument-bug reds: red-path reachability, NOT defect detection**, so they do not earn live grade | live PASS for these checks honestly prints PASS-UNPROVEN until a live campaign (or a deliberate reintroduction, e.g. re-running the `b5bd232` notifhost through the full suite → P3/P4a red) observes a real red through the protocol path |
| `fwd_count` / `fwd_attempts` (a0-lib delivery detectors) | 2026-09-05 | **EARNED, two ways.** (1) Fixture negative: `fwd_selftest` (a0-lib.sh) feeds an undelivered window (`FWD_RTT ... ok=-2`, `SENT ...: FAIL`, `SENT coalesced x4: FAIL`, a title-embedded `ok=1` forgery) — fwd_count must read 0 and mid-line forgeries must not match (patterns anchored at `^HH:MM:SS `). (2) ORGANIC red on first execution: the delivered fixture read 2, not 5 — `paste\|bc` returned empty because **bc is not installed on this rig**, which also proves the OLD harness's `SENT coalesced` sum (a0-toast-bridge.sh P7, a0-p3-toast-split.sh Q4) was silently always 0. Fixed to awk, selftest green. Replaces the `SENT id=.*OK`-only greps that false-FAILed P4a/P6b/P7 on 2026-09-05 when BLog's share-READ-only open dropped SENT lines racing the 5133293 shadow worker's CLASSIFY (ids 15/20/22: FWD_RTT ok=1 + dom0 Dismissed callback, no SENT) | `fwd_selftest` runs in-harness before P1 verdicts (a0-toast-bridge.sh P1c, a0-selftest.sh S0b) and aborts INSTRUMENT on mismatch; replay of the failed run's own evidence: `p4-blog.txt` fwd_count=1, `p7-blog.txt` fwd_count=5, `p4c-blog.txt` fwd_attempts=0, full log fwd_count=9 = exactly the 9 acked forwards |
...
|---|---|---|
| `emulated-unplugged` / `standalone-pvnic-seeded` / `template-pvnic-seeded` | **CLAIM NARROWED 2026-08-31** — the negative is unreachable via the LATCH, but REACHABLE via the PAYLOAD: a consistent setup tree with `pvnic-selfprime.ps1` removed (and only its `SHA256SUMS.txt` line dropped) installed cleanly on a disposable clone and reported **`pvnic_prime: not in payload`**, `stage2-install / ok:true`. So the defect state DOES exist. It still earns no proof, because the exact-match audit shows **no harness emits these verdicts** — they are hand-recorded observations. Original latch finding, still valid: | two attempts to unseed `XEN\Unplug NICS=0` and reboot - the second with BOTH latch tasks disabled - came back `NICS=1, EMULATED_LEFT 0` every time. The PV stack re-seeds below the task layer on every boot, so the defect cannot be planted; it would need a guest with the PV drivers removed |
