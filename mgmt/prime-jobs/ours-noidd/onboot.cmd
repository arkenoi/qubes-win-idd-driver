...
rem /noidd IS THE VARIABLE UNDER TEST. Everything else in this job is byte-identical to
rem prime-jobs/ours. The IDD activation disables the emulated VGA (PCI\VEN_1234&DEV_1111 ->
rem code 22) IMMEDIATELY BEFORE this stage-2 reboot, and a live wedge captured 2026-09-10
rem showed Xen spinning in flush_area_mask beneath hap_track_dirty_vram <- dm_op - i.e. the
rem device model TEARING DOWN its dirty-VRAM tracking, which is what disabling the display
rem adapter causes. /noidd leaves the BDA in place and never triggers that teardown, so if
rem the stall is caused by it, this arm should not stall and the default arm should.
call C:\qwtsetup\install.cmd /auto /reboot /noidd /autologon:qubes >> %LOG% 2>&1
